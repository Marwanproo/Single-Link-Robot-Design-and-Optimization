...
{
    cout <<"Is the length of the link can be optemized (1) or not (2) "<<endl;
    int i;
    i = checkValidation(1,2);
    ...
    {
            if(i == 1) l.increase_Circle_volume(m);
            if(i == 1)l.increase_Rectangle_volume(m);
        ...
        {
            if(i == 1)l.reduce_Circle_volume(m);
            if(i == 1)l.reduce_Rectangle_volume(m);
...
void setMotorsDatabase(vector<Motor>& m){
    ...
    string motor_name; double motor_torque; double motor_speed; double motor_mass; double motor_diameter; double motor_width;
...
void sorting(vector<MotorGearboxCompination>& mg,int i){
    ...
    if (min_selector = 1){
        //cost
        for(int j = 0 ; j < mg.size()-1;++j){
        for(int i = 1 ; i < mg.size()-1;++i){
